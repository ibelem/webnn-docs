...
> <InfoIcon /> You can insert both meta tags into the `<head>` section of HTML document, one for each browser's token. The browsers are designed to ignore tokens they don't understand, so you can safely include both.
